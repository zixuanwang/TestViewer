...
cv::Mat GeometricVerifier::findHomography(const cv::Mat& srcPoints,
		const cv::Mat& dstPoints, std::vector<uchar>& inliers) {
	...
	srand (time(NULL));float
	thresholdSquare = 4.0f;
	...
	for (int iteration = 0; iteration < 500; ++iteration) {
		...
		for (int i = 0; i < count; ++i) {
			...
			float _x = (float) modelArray[0] * srcX
					+ (float) modelArray[1] * srcY + (float) modelArray[2];
			float _y = (float) modelArray[3] * srcX
					+ (float) modelArray[4] * srcY + (float) modelArray[5];
			float _z = (float) modelArray[6] * srcX
					+ (float) modelArray[7] * srcY + (float) modelArray[8];
			float _invz = 1.0f / _z;
			float u = _x * _invz;
			float v = _y * _invz;
			float dx = u - dstX;
