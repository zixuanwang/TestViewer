...
cv::Mat PlanarObjectTracker::findHomography(
	...
	cv::Mat homography = verifier.findHomography(cv::Mat(srcPoints),
			cv::Mat(dstPoints), inliers);
	int inlierCount = 0;
...
void PlanarObjectTracker::track(cv::Mat& image) {
	...
	if (objectArray.size() > 90) {
		...
		solveGaussNewton(objectPoints, imagePoints, mIntrinsicMatrix,
...
bool PlanarObjectTracker::getTemplatePatch(cv::Mat* pTemplatePatch,
		const cv::Mat& invH, const cv::Point& point) {
	...
	for (int x = -1 * halfWindowSize, i = 0; x < halfWindowSize; ++x, ++i) {
		for (int y = -1 * halfWindowSize, j = 0; y < halfWindowSize; ++y, ++j) {
			double du, dv;
			projectPoint(&du, &dv, ptr, (double) point.x + x,
					(double) point.y + y);
			int u = (int) (du + 0.5f);
			int v = (int) (dv + 0.5f);
			if (u > 0 && u < mTemplateImage.cols && v > 0
					&& v < mTemplateImage.rows) {
				pTemplatePatch->at < uchar > (j, i) = mTemplateImage.at < uchar
						> (v, u);
			} else {
				return false;
			}
...
void PlanarObjectTracker::projectPoint(double* pX, double* pY,
		const double h[9], double x, double y) {
...
float PlanarObjectTracker::match(const cv::Mat& templatePatch,
		const cv::Mat& targetPatch, const cv::Point& srcPoint,
		cv::Point* pDstPoint) {
	cv::Point maxPoint;
...
float PlanarObjectTracker::zncc(const cv::Mat& image,
	...
	for (int i = 1; i <= image.rows; ++i) {
		...
		for (int j = 1; j <= image.cols; ++j) {
			...
			integralSum[i][j] = integralSum[i - 1][j] + integralSum[i][j - 1]
					- integralSum[i - 1][j - 1] + val;
			integralSquareSum[i][j] = integralSquareSum[i - 1][j]
					+ integralSquareSum[i][j - 1]
					- integralSquareSum[i - 1][j - 1] + val * val;
		}
	}
	...
	for (int y1 = 0; y1 < resultRows; ++y1) {
		for (int x1 = 0; x1 < resultCols; ++x1) {
			...
			int locationSum = integralSum[y2][x2] + integralSum[y1][x1]
					- integralSum[y1][x2] - integralSum[y2][x1];
			int locationSquareSum = integralSquareSum[y2][x2]
					+ integralSquareSum[y1][x1] - integralSquareSum[y1][x2]
					- integralSquareSum[y2][x1];
			int crossSum = 0;
			for (int i = 0; i < templateImage.rows; ++i) {
	...
	float* iter = std::max_element(scoreArray,
			scoreArray + resultRows * resultCols);
...
double PlanarObjectTracker::solveGaussNewton(const cv::Mat& objectPoints,
		const cv::Mat& imagePoints, const cv::Mat& cameraMatrix,
		const cv::Mat& distCoeffs, cv::Mat& rvec, cv::Mat& tvec) {
	cv::Mat undistortImagePoints;
	cv::undistortPoints(imagePoints, undistortImagePoints, cameraMatrix,
			distCoeffs);
	double error = 0.0f;
	cv::Mat RMat;
	...
	for (int iteration = 0; iteration < 2; ++iteration) {
		...
		for (int i = 0; i < imagePoints.rows; ++i) {
			...
			// compute Jacobian
			double x = currentTransform.R[0] * (double) objectPtr[0]
					+ currentTransform.R[1] * (double) objectPtr[1]
					+ currentTransform.R[2] * (double) objectPtr[2]
					+ currentTransform.t[0];
			double y = currentTransform.R[3] * (double) objectPtr[0]
					+ currentTransform.R[4] * (double) objectPtr[1]
					+ currentTransform.R[5] * (double) objectPtr[2]
					+ currentTransform.t[1];
			double invz = 1.0f
					/ (currentTransform.R[6] * (double) objectPtr[0]
							+ currentTransform.R[7] * (double) objectPtr[1]
							+ currentTransform.R[8] * (double) objectPtr[2]
							+ currentTransform.t[2]);
			double u = x * invz;
			double v = y * invz;
			double uv = u * v;
			// compute error
			...
			double jArray[12] = { invz, 0.0f, -1.0f * u * invz, -1.0f * uv, 1.0f
					+ u * u, -1.0f * v, 0.0f, invz, -1.0f * v * invz, -1.0f
					- v * v, uv, u };
			double jTArray[12] = { invz, 0.0f, 0.0f, invz, -1.0f * u * invz,
					-1.0f * v * invz, -1.0f * uv, -1.0f - v * v, 1.0f + u * u,
					uv, -1.0f * v, u };
