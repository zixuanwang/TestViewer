...
public:
	...
	bool getTemplatePatch(cv::Mat* pTemplatePatch, const cv::Mat& invH,
			const cv::Point& point);
	void projectPoint(double* pX, double* pY, const double h[9], double x,
			double y);
	...
	float match(const cv::Mat& templatePatch, const cv::Mat& targetPatch,
			const cv::Point& srcPoint, cv::Point* pDstPoint);
	...
	float zncc(const cv::Mat& image, const cv::Mat& templateImage,
			cv::Point* pMaxPoint);
	double solveGaussNewton(const cv::Mat& objectPoints,
